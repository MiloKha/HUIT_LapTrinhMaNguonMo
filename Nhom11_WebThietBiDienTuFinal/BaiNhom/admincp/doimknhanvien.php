...
if(isset($_POST['doimatkhau'])){
		...
		$sql = "SELECT * FROM tbl_nhanvien WHERE taikhoan='".$_SESSION['dangnhap']."' AND matkhau='".$matkhau_cu."' LIMIT 1";
		$row = $conn->query($sql);
		$count = $row->fetchColumn();
		if($count>0){
            if($matkhau_moi == $nhaplai_matkhau_moi){
			$sql_update="UPDATE tbl_nhanvien SET matkhau ='".$matkhau_moi."' WHERE taikhoan='".$_SESSION['dangnhap']."'";
